...
public class LevelLoader {
	...
	public void loadLevel(List <Ship> ship, Robot robot, SelfDestructButton button, List <Alien> aliens, 
			...
			List<Instructions> instructions, List<Rotator> rotators, ControlPanel controlpanel) throws XmlPullParserException, IOException{
		...
		try {
			...
			while (parser.next() != XmlPullParser.END_TAG) {
				...
				if (name.equals("ship")) 
					...
					rotators.add(readRotators(parser));    	
	...
	private Rotator readRotators(XmlPullParser parser) throws XmlPullParserException, IOException{
		parser.require(XmlPullParser.START_TAG, ns, "rotator");
		float x = 0.0f; 
		float y = 0.0f;
		int dir = 0; 
